...
| `query_id`        | VARCHAR(200)                         | **НОВОЕ**: ID запроса для получения данных через `/api/data`. Используется для компонентов типа `table`, `button`, `header`. Связывает компонент с запросом в `config.component_queries.query_id`. Пример: `header` компонент имеет `query_id = 'header_dates'` для получения дат периодов. Возвращается в layout JSON как `queryId`. |
| `data_source_key` | VARCHAR(200)                         | Ключ источника данных для KPI карточек. Для карточек (`component_type='card'`) содержит `tech_kpi_name` — техническое имя KPI. Используется для сопоставления KPI метрик с компонентами через JOIN в `mart.v_kpi_all`. Пример: `capital_card` имеет `data_source_key = 'capital'`. |
...
- `header` - компонент header с `query_id = 'header_dates'` для получения дат периодов
- `assets_table` - таблица активов с `query_id = 'assets_table'`
- `capital_card` - карточка капитала с `data_source_key = 'capital'` (tech_kpi_name)
- `button_assets_table_cfo` - кнопка группировки с `query_id = 'assets_table'`

**Использование полей:**

| Тип компонента | `query_id` | `data_source_key` |
|----------------|------------|-------------------|
| `header` | ID запроса для `/api/data` | — |
| `table` | ID запроса для `/api/data` | — |
| `button` | ID запроса для `/api/data` | — |
| `card` | — | `tech_kpi_name` для KPI mapping |

**Использование:** В SQL Builder при получении layout через `/api/data?query_id=layout` и KPI метрик через `/api/data?query_id=kpis`. Компоненты с `query_id` получают данные через `/api/data` endpoint. Карточки используют `data_source_key` для сопоставления с KPI данными.
...
| `display_order`      | INTEGER, DEFAULT 0                   | Порядок отображения поля в компоненте. Определяет порядок колонок в таблице.                                                                                                                                                                                                                                                                                                                          |
| `is_active`          | BOOLEAN, DEFAULT true                | Флаг активности поля. Неактивные поля не используются в компоненте.                                                                                                                                                                                                                                                                                                                                   |
| `field_type`         | VARCHAR(50)                          | **НОВОЕ (миграция 030)**: Тип поля: 'dimension' (измерение для группировки), 'measure' (метрика для агрегации), 'calculated' (вычисляемое на фронте), 'attribute' (атрибут).                                                                                                                                                                                                                          |
| `calculation_config` | JSONB                                | **НОВОЕ (миграция 030)**: Конфиг для вычисляемых полей. Формат: `{"type": "percent_change", "valueField": "value", "baseField": "ppValue"}`.                                                                                                                                                                                                                                                          |
| `aggregation`        | VARCHAR(50)                          | **НОВОЕ (миграция 030)**: Тип агрегации для measure: 'sum', 'avg', 'min', 'max', 'count'.                                                                                                                                                                                                                                                                                                             |
...
VIEW для получения списка дат периодов с флагами p1/p2/p3.

**Назначение:** Предоставляет список всех доступных дат периодов из `mart.v_kpi_all` с автоматическим определением флагов для последней даты (p1), предпоследней даты (p2) и последней даты предыдущего года (p3). Используется через SQL Builder для получения данных через `/api/data?query_id=header_dates`.
...
| Поле         | Тип     | Описание                                                                                                                                    |
| ------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `period_date` | DATE    | Дата периода (YYYY-MM-DD). Уникальные даты из `mart.v_kpi_all`, отсортированные по убыванию (от новых к старым).                          |
| `is_p1`      | BOOLEAN | Флаг последней даты (p1). `true` для самой новой даты в списке, `false` для остальных.                                                    |
| `is_p2`      | BOOLEAN | Флаг предпоследней даты (p2). `true` для второй по новизне даты, `false` для остальных.                                                     |
| `is_p3`      | BOOLEAN | Флаг последней даты предыдущего года (p3). `true` для последней даты года, предшествующего году p1, `false` для остальных.                 |

**Логика расчета флагов:**

1. **is_p1:** Устанавливается в `true` для самой новой даты (первая строка после сортировки по убыванию)
2. **is_p2:** Устанавливается в `true` для второй по новизне даты (вторая строка после сортировки)
3. **is_p3:** Устанавливается в `true` для последней даты года, предшествующего году p1

**Источник данных:** VIEW получает уникальные даты из `mart.v_kpi_all` через `SELECT DISTINCT period_date`.

**Пример данных:**

```
period_date  | is_p1 | is_p2 | is_p3
--------------+-------+-------+-------
2025-12-31   | true  | false | false
2025-11-30   | false | true  | false
2025-10-31   | false | false | false
...
2024-12-31   | false | false | true
2024-11-30   | false | false | false
```

**Использование:** В SQL Builder через конфиг `query_id='header_dates'` для получения списка дат периодов. API возвращает данные в camelCase формате (`periodDate`, `isP1`, `isP2`, `isP3`) для использования на фронтенде в компоненте DatePicker.

**См. также:** [Миграция 056](/database/migrations) - создание VIEW `mart.v_p_dates`
