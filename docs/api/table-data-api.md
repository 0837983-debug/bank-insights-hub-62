...
Группировка позволяет агрегировать данные по определенным полям. Доступные опции группировки определяются через кнопки-компоненты в layout конфигурации. Каждая кнопка имеет `settings.groupBy` для указания поля группировки.

**Примечание:** Устаревшее поле `groupableFields` заменено на кнопки-компоненты. См. [Layout Architecture](/architecture/layout) для деталей.
