...
### 6. Минимальная обработка данных на фронтенде

Проект следует принципу **минимальной обработки данных на фронтенде**.

**Все расчеты выполняются на backend:**
- ppChange, ytdChange, percentage вычисляются в сервисах mart
- Агрегация данных из БД
- Формирование готовых структур данных

**Frontend только:**
- Форматирует данные для отображения (`formatValue`)
- Строит иерархическую структуру UI из плоских данных (`transformTableData`)
- Пересчитывает метрики для групп при необходимости (агрегация групп в UI)
