-- Миграция 033: Обновить layout view для field_type
-- Дата: 2026-02-03
-- Задача: FIELD_TYPE_REFACTOR этап 2.1
-- 
-- Изменения:
-- 1. data_type вместо field_type для типа данных (column_type)
-- 2. Добавить field_type как fieldType (dimension/measure/calculated/attribute)
-- 3. Добавить calculation_config как calculationConfig
-- 4. Добавить aggregation
-- 5. НЕ возвращать is_dimension, is_measure (deprecated!)
...
columns_base AS (
  ...
  -- ИЗМЕНЕНО: data_type вместо field_type, добавлены field_type, calculation_config, aggregation
  SELECT
    cb.layout_id,
    cb.section_id,
    cb.component_id,
    ...
    cf.data_type AS column_type,  -- ИЗМЕНЕНО: было field_type
    cf.format_id AS column_format,
    cf.description AS column_description,
    cf.field_type AS column_field_type,  -- НОВОЕ: dimension/measure/calculated/attribute
    cf.calculation_config AS column_calculation_config,  -- НОВОЕ
    cf.aggregation AS column_aggregation,  -- НОВОЕ
...
sub_columns_base AS (
  ...
  SELECT
    ...
    cf.data_type AS sub_column_type,  -- ИЗМЕНЕНО: было field_type
    cf.format_id AS sub_column_format,
    cf.description AS sub_column_description,
    cf.field_type AS sub_column_field_type,  -- НОВОЕ
    cf.calculation_config AS sub_column_calculation_config,  -- НОВОЕ
    cf.aggregation AS sub_column_aggregation,  -- НОВОЕ
          ...
          SELECT COALESCE(
            jsonb_agg(
              jsonb_build_object(
                ...
                -- НОВОЕ: fieldType вместо isDimension/isMeasure
                  ...
                  SELECT COALESCE(
                    jsonb_agg(
                      jsonb_build_object(
                        ...
                        -- НОВОЕ: fieldType и calculationConfig для sub_columns
...
COMMENT ON VIEW config.layout_sections_json_view IS 'Агрегированный view с готовой структурой sections (jsonb). Включает fieldType, calculationConfig, aggregation. НЕ включает deprecated isDimension/isMeasure.';
