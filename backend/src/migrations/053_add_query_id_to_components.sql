-- Миграция 053: Добавить колонку query_id в config.components
-- Дата: 2026-02-09
-- Задача: J3_QUERY_ID_AND_KPI_MAPPING - Этап 1
-- 
-- Изменения:
-- 1. Добавить колонку query_id в config.components
-- 2. Заполнить query_id для table/button/header из data_source_key
-- 3. Обновить layout_sections_json_view для отдачи queryId

-- =============================================================================
-- Шаг 1: Добавить колонку query_id
-- =============================================================================

ALTER TABLE config.components ADD COLUMN IF NOT EXISTS query_id VARCHAR(100);

COMMENT ON COLUMN config.components.query_id IS 'ID запроса для getData (для table/button/header). Отделено от data_source_key который используется как ключ внутри KPI-набора.';

-- =============================================================================
-- Шаг 2: Заполнить query_id для существующих компонентов
-- =============================================================================

-- Для table, button, header — query_id = data_source_key (текущее поведение)
UPDATE config.components 
SET query_id = data_source_key 
WHERE component_type IN ('table', 'button', 'header') 
  AND data_source_key IS NOT NULL
  AND query_id IS NULL;

-- =============================================================================
-- Шаг 3: Обновить VIEW для отдачи queryId
-- =============================================================================
...
components_base AS (
  ...
  SELECT
    ...
    c.query_id AS component_query_id,  -- НОВОЕ
...
buttons_base AS (
  ...
  SELECT
    ...
    button_c.query_id AS button_query_id,  -- НОВОЕ
...
sub_columns_base AS (
  ...
  -- Включает display_group и is_default для calculated полей
  SELECT
    cb.layout_id,
    cb.section_id,
    cb.component_id,
    ...
    cf.display_group AS sub_column_display_group,
    cf.is_default AS sub_column_is_default,
...
base AS (
  ...
  FROM (
    ...
    SELECT
      ...
      NULL::VARCHAR(100) AS button_query_id,
    ...
    SELECT
      ...
      NULL::VARCHAR(100) AS component_query_id,
...
COMMENT ON VIEW config.layout_sections_json_view IS 'Агрегированный view с готовой структурой sections (jsonb). Включает fieldType, calculationConfig, aggregation, displayGroup, isDefault, queryId.';
