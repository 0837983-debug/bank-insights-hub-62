...
-- Добавляет кнопку "Активы" для table_balance с отдельным query_id.
-- Запрос кнопки повторяет table_balance, но с фильтром tech_class = 'АКТИВЫ'.

BEGIN;

-- 1) Конфиг SQL Builder для кнопки "Активы"
INSERT INTO config.component_queries (query_id, title, config_json, wrap_json, is_active)
VALUES (
  'table_balance_assets',
  'Данные таблицы баланса: только активы',
  '{
    "from": {
      "schema": "mart",
      "table": "balance"
...
VALUES (
  'button_table_balance_assets',
  'button',
  'Активы',
  'Активы',
  'Фильтр таблицы баланса по tech_class = АКТИВЫ',
  'table_balance_assets',
  'table_balance_assets',
  '{"fieldId":"tech_class","groupBy":"tech_class","fixedValue":"АКТИВЫ"}'::jsonb,
  TRUE
)
...
AND NOT EXISTS (
  ...
  WHERE lcm.layout_id = 'main_dashboard'
    AND lcm.component_id = 'button_table_balance_assets'
...
WHERE layout_id = 'main_dashboard'
  AND component_id = 'button_table_balance_assets'
