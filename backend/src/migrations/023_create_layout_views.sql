-- Миграция 023: Создание view для layout (sections_json)
-- Дата: 2025-01-XX

-- Удаляем старые view, которые больше не нужны
DROP VIEW IF EXISTS config.layout_formats_view CASCADE;
DROP VIEW IF EXISTS config.layout_header_view CASCADE;
DROP VIEW IF EXISTS config.layout_sections_view CASCADE;
DROP VIEW IF EXISTS config.layout_sections_agg_view CASCADE;
DROP VIEW IF EXISTS config.layout_sections_json_view CASCADE;

-- ============================================
-- VIEW: config.layout_sections_json_view
-- ============================================
-- Агрегированный view, собирающий структуру sections в JSON для использования через SQL Builder
-- Выход: layout_id, section_id, section (jsonb)
...
columns_base AS (
  ...
  SELECT
    ...
    cf.field_type AS column_type,
    cf.format_id AS column_format,
    cf.description AS column_description,
    cf.is_dimension AS column_is_dimension,
    cf.is_measure AS column_is_measure,
...
sub_columns_base AS (
  ...
  SELECT
    ...
    cf.field_type AS sub_column_type,
...
layout_sections_agg_data AS (
  ...
  NULL::BOOLEAN AS column_is_dimension,
  NULL::BOOLEAN AS column_is_measure,
...
SELECT
  ...
  colb.column_is_dimension,
  colb.column_is_measure,
          ...
          SELECT COALESCE(
            jsonb_agg(
              jsonb_build_object(
                ...
                'isDimension', c.column_is_dimension,
                'isMeasure', c.column_is_measure,
                'sub_columns', (
                  SELECT COALESCE(
                    jsonb_agg(
                      jsonb_build_object(
...
COMMENT ON VIEW config.layout_sections_json_view IS 'Агрегированный view с готовой структурой sections (jsonb) для использования через SQL Builder.';
